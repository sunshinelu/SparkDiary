...
:\ :[B](z: B)(op: (A, B) ⇒ B): B 与foldRight等价
    ...
    val right1 = List(4, 5, 6)

    //以下操作等价
    left1 ++ right1 // List(1,2,3,4,5,6)
    left1 ++: right1 // List(1,2,3,4,5,6)
    right1.++:(left1) // List(1,2,3,4,5,6)
    right1.:::(left1) // List(1,2,3,4,5,6)

    //以下操作等价
    0 +: left1 //List(0,1,2,3)
    left1.+:(0) //List(0,1,2,3)

    //以下操作等价
    left1 :+ 4 //List(1,2,3,4)
    left1.:+(4) //List(1,2,3,4)

    //以下操作等价
    0 :: right1 //List(0,1,2,3)
    left1.::(0) //List(0,1,2,3)
    /*
    二、常用变换操作
     */
...
foldRight: foldRight[B](z: B)(op: (A, B) ⇒ B): B 带有初始值的reduceRight
     */
    val nums5 = List(2, 3, 4)
    val sum = nums5.fold(1)(_ + _) // = 1+2+3+4 = 9

    val nums5_2 = List(2.0, 3.0)
    val result1 = nums5_2.foldLeft(4.0)(math.pow) // = pow(pow(4.0,2.0),3.0) = 4096
    val result2 = nums5_2.foldRight(1.0)(math.pow) // = pow(1.0,pow(2.0,3.0)) = 8.0
...
sortWith： sortWith(lt: (A, A) ⇒ Boolean): List[A] 使用自定义的比较函数进行排序
     ...
     */
    val nums9_1 = List(1, 2, 3)
    val nums9_2 = List(2, 3, 4)
    val diff1 = nums9_1 diff nums9_2 // List(1)
    val diff2 = nums9_2.diff(nums9_1) // List(4)
    val union1 = nums9_1 union nums2 // List(1,2,3,2,3,4)
    val union2 = nums9_2 ++ nums9_1 // List(2,3,4,1,2,3)
    val intersection = nums9_1 intersect nums9_2 //List(2,3)
     ...
     */
    val list10 = List("A", "B", "C", "A", "B")
    val distincted = list10.distinct // List("A","B","C")
     ...
     */
    val data11 = List(("HomeWay", "Male"), ("XSDYM", "Femail"), ("Mr.Wang", "Male"))
    val group1 = data11.groupBy(_._2) // = Map("Male" -> List(("HomeWay","Male"),("Mr.Wang","Male")),"Female" -> List(("XSDYM","Femail")))
    val group2 = data11.groupBy { case (name, sex) => sex } // = Map("Male" -> List(("HomeWay","Male"),("Mr.Wang","Male")),"Female" -> List(("XSDYM","Femail")))
    val fixSizeGroup = data11.grouped(2).toList // = Map("Male" -> List(("HomeWay","Male"),("XSDYM","Femail")),"Female" -> List(("Mr.Wang","Male")))
...
由一个初始值开始，从左向右，进行积累的op操作，这个比较难解释，具体的看例子吧。
     */
    val nums12 = List(1, 2, 3)
    val result = nums12.scan(10)(_ + _) // List(10,10+1,10+1+2,10+1+2+3) = List(10,11,12,13)
    ...
    */
    ...
    val nums13 = List(1.0, 2.0, 3.0)
    val result13_1 = nums13.scanLeft(2.0)(math.pow) // List(2.0,pow(2.0,1.0), pow(pow(2.0,1.0),2.0),pow(pow(pow(2.0,1.0),2.0),3.0) = List(2.0,2.0,4.0,64.0)
    val result13_2 = nums13.scanRight(2.0)(math.pow) // List(2.0,pow(3.0,2.0), pow(2.0,pow(3.0,2.0)), pow(1.0,pow(2.0,pow(3.0,2.0))) = List(1.0,512.0,9.0,2.0)
     ...
     */
    val nums14 = List(1, 1, 1, 1, 4, 4, 4, 4)
    val left = nums14.take(4) // List(1,1,1,1)
    val right = nums14.takeRight(4) // List(4,4,4,4)
    val headNums = nums14.takeWhile(_ == nums.head) // List(1,1,1,1)
...
dropWhile: dropWhile(p: (A) ⇒ Boolean): List[A] 从左向右丢弃元素，直到条件p不成立
     */
    val nums15 = List(1, 1, 1, 1, 4, 4, 4, 4)
    val left15 = nums15.drop(4) // List(4,4,4,4)
    val right15 = nums15.dropRight(4) // List(1,1,1,1)
    val tailNums = nums15.dropWhile(_ == nums15.head) // List(4,4,4,4)
    /*
    ...
    val nums16 = List(1, 1, 1, 2, 3, 2, 1)
    val (prefix16_1, suffix16_1) = nums16.span(_ == 1) // prefix = List(1,1,1), suffix = List(2,3,2,1)
    val (prefix16_2, suffix16_2) = nums16.splitAt(3) // prefix = List(1,1,1), suffix = List(2,3,2,1)
    val (prefix16_3, suffix16_3) = nums16.partition(_ == 1) // prefix = List(1,1,1,1), suffix = List(2,3,2)
    ...
    val nums17 = List(1, 1, 1)
    val padded = nums17.padTo(6, 2) // List(1,1,1,2,2,2)
    /*
     ...
     */
    val nums18 = List(1, 1, 3)
    val combinations = nums18.combinations(2).toList //List(List(1,1),List(1,3))
    val permutations = nums18.permutations.toList // List(List(1,1,3),List(1,3,1),List(3,1,1))
    ...
    val nums19 = List(1, 2)
    val zipped = alphabet zip nums19 // List(("A",1),("B",2))
    val zippedAll = alphabet.zipAll(nums19, "*", -1) // List(("A",1),("B",2),("C",-1))
    ...
    val nums20 = List(1, 2, 3, 4, 5)
    val sliced = nums20.slice(2, 4) //List(3,4)
    /*
    21.sliding
    ...
    val nums21 = List(1, 1, 2, 2, 3, 3, 4, 4)
    val groupStep2 = nums21.sliding(2, 2).toList //List(List(1,1),List(2,2),List(3,3),List(4,4))
    val groupStep1 = nums21.sliding(2).toList //List(List(1,1),List(1,2),List(2,2),List(2,3),List(3,3),List(3,4),List(4,4))
     ...
     */
    val nums22 = List(1, 2, 3, 3)
    val fixed = nums22.updated(3, 4) // List(1,2,3,4)
