package com.evayInfo.Inglory.SparkDiary.ml.regression
...
object LinearRegressionDemo1 {
  ...
  def main(args: Array[String]) {
    ...
    // Area州面积
    val conf = new SparkConf().setAppName(s"LinearRegressionDemo1").setMaster("local[*]").set("spark.executor.memory", "2g")
    ...
    val dataList: List[(Double, Double, Double, Double, Double, Double, Double, Double)] = List(
      ...
      (376, 4566, 0.6, 70.29, 6.9, 62.9, 173, 97203)).map{x =>
      (x._1.toDouble,x._2.toDouble,x._3.toString.toDouble,x._4.toDouble,x._5.toString.toDouble,x._6.toString.toDouble,x._7.toDouble,x._8.toDouble)}
