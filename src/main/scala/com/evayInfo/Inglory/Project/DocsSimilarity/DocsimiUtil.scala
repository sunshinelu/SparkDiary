...
object DocsimiUtil {
  ...
  def getYlzxSegRDD(ylzxTable: String, year: Int, sc: SparkContext): RDD[YlzxSegSchema2] = {
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      map(x => {
        ...
        YlzxSegSchema(x._1, x._2, x._3, x._4, time, x._6, segWords)
      ...
      YlzxSegSchema2(id, urlID, title, label, time, websitename, segWords)
  ...
  case class YlzxSegSchema3(id: Long, urlID: String, title: String, label: String, time: String, websitename: String,
                            content: String, segWords: Seq[String])

  def getYlzxSeg2RDD(ylzxTable: String, year: Int, sc: SparkContext): RDD[YlzxSegSchema3] = {
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      val websitename = x._1.websitename
      val segWords = x._1.segWords
      YlzxSegSchema3(id, urlID, title, label, time, websitename, content, segWords)
    }).filter(x => null != x.segWords).filter(_.segWords.size > 1) //.randomSplit(Array(0.1,0.9))(0)
