 ...
 */
object WuUtil extends Serializable {
  ...
  case class LogSchema(CREATE_BY_ID: String, CREATE_TIME: Long, REQUEST_URI: String, PARAMS: String)

  case class LogSchema2(userString: String, itemString: String, CREATE_TIME: Long, value: Double)

  def getLogsRDD(logsTable: String, sc: SparkContext): RDD[LogSchema2] = {
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      map { x => {
        ...
        LogSchema(userID, creatTimeL, requestURL, parmas)
      }
      ...
      map(x => {
        ...
        LogSchema2(userID, urlString, time, rating)
      ...
      map(x => {
        ...
        val rating = time match {
          case x if (x >= DateUtil.get3Dasys()) => 0.9 * value
          case x if (x >= DateUtil.get7Dasys() && x < DateUtil.get3Dasys()) => 0.8 * value
          case x if (x >= DateUtil.getHalfMonth() && x < DateUtil.get7Dasys()) => 0.7 * value
          case x if (x >= DateUtil.getOneMonth() && x < DateUtil.getHalfMonth()) => 0.6 * value
          case x if (x >= DateUtil.getSixMonth() && x < DateUtil.getOneMonth()) => 0.5 * value
          case x if (x >= DateUtil.getOneYear() && x < DateUtil.getSixMonth()) => 0.4 * value
          case x if (x < DateUtil.getOneYear()) => 0.3 * value
          case _ => 0.0
        }

        //val rating = rValue(time, value)
        LogSchema2(userString, itemString, time, rating)
      })
  ...
  def getYlzxYRDD2(ylzxTable: String, year: Int, sc: SparkContext): RDD[YlzxSchema] = {
    //定义时间格式
    ...
    val hbaseRDD = hBaseRDD.map { case (k, v) => {
      ...
      YlzxSchema(x._1, x._2, x._3, time, x._4, x._5, content) //, x._7)
    })
