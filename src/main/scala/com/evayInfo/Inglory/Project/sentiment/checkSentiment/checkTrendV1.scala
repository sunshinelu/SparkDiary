...
//    val url2 = "jdbc:mysql://192.168.37.18:3306/bbs?useUnicode=true&characterEncoding=UTF-8"
  ...
  val masterDF = allDF.drop("content")//.dropDuplicates(Array("articleId"))
  val slaveDF = allDF.select("articleId", "content")//.dropDuplicates(Array("articleId"))

  println("allDF的数量为：" + allDF.count())//articleDF的数量为：29457
  println("allDF中ARTICLEID列除重后的数量为：" + allDF.dropDuplicates(Array("ARTICLEID")).count())
  println("allDF除重后的数量为：" + allDF.dropDuplicates().count())
  println("masterDFF的数量为:" + masterDF.count)//contentDF的数量为:29456
  println("masterDF除重后的数量为:" + masterDF.dropDuplicates().count) //
  println("masterDF中ARTICLEID列除重后的数量为:" + masterDF.dropDuplicates(Array("ARTICLEID")).count) //
  println("articleDF的数量为:" + slaveDF.count)
  println("articleDF除重后的数量为:" + slaveDF.dropDuplicates().count)
  println("articleDFF中ARTICLEID列除重后的数量为:" + slaveDF.dropDuplicates(Array("ARTICLEID")).count)


allDF的数量为：29868
allDF中ARTICLEID列除重后的数量为：29868
allDF除重后的数量为：29868
masterDFF的数量为:29868
masterDF除重后的数量为:29868
masterDF中ARTICLEID列除重后的数量为:29868
articleDF的数量为:29868
articleDF除重后的数量为:29868
articleDFF中ARTICLEID列除重后的数量为:29868
   */
    /*
articleDF的数量为：29884
articleDF中ARTICLEID列除重后的数量为：29884
articleDF除重后的数量为：29884
contentDF的数量为:30936
contentDF除重后的数量为:29895
allDF的数量为:29868
allDF除重后的数量为:29868
allDF中ARTICLEID列除重后的数量为:29868
     */
    /*
      val t_df_1 = contentDF.select("ARTICLEID").except(articleDF.select("ARTICLEID"))
