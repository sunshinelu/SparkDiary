...
import org.apache.spark.sql.functions.{bround, current_timestamp, date_format, lit, sum, udf, max => f_max}
...
object TongShiRelationHBase {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val temp_7 = temp_6.select("id_2", "id_1", "company", "degree").
      toDF("id_1", "id_2", "company", "degree")

    val temp_8 = temp_7.union(temp_6).groupBy("id_1", "id_2", "company").
      agg(f_max("degree")).
      withColumnRenamed("max(degree)", "degree")
    ...
    val result_df = ds3.select("id_1", "name_1", "id_2", "name_2", "relation", "company", "degree", "create_time", "update_time").
